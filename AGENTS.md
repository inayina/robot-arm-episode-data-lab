...
| **Sensor Fusion** | `sensor_fusion_node`（**experimental**） | 多源对齐、接触估计；相机像素未用 |
...
- SmolVLA **S3 Ready** / **S3 Hold** 等同于任务成功 / 可自行进 Isaac（Recovery v3 的 LoRA 已完成，但 Ready/Hold 标签本身仍不代表成功）；
- SmolVLA Recovery v3 的 **open-loop Pass**、有界 S4 的 **interface 5/5** 或 `ran_isaac=true` 等同于任务成功 / 在线自主抓取 / Sim2Real / 真机；
- 有界 S4 首轮近黑场景的 reach 3/5 · grasp 1/5 为权威或「部分成功」（已被修光复测证伪，标注 Superseded）。
...
- 不得因 SmolVLA S2 接口 Pass、S3 Ready 或 S3 Hold 而进入 Isaac；已执行的有界 S4（≤5 seeds）是**一次性人工批准**，不得据此自动再跑 Isaac、扩种子或重训；
...
- 2026-07-24 Recovery v3：`eval_gate_v2` prospective 仍为历史 **Hold**（开爪边严重度）。同日用户批准硬约束可修订：冻结执行语义 `eval_gate_v3`；新采 seeds 70–74 / 2,593 帧 eval-only；RTX 4090 D canonical prospective **Pass**（EE `0.0253 m`、grip BA `0.9943`、close-edge beyond-ε `0.386%`、clip MAE `0.00692`、分类/时序 0；开爪边过冲仅 diagnostic）。该 Pass 仅代表**专家状态分布上的 first-action 离线判定**，`queued_diagnostic` 永不具备 canonical Pass 资格。有界 Isaac S4（≤5 seeds）已在本机 RTX PRO 500 跑完：`ran_isaac=true`；policy interface 5/5；**权威产物为修光后复测** `evidence/smolvla_s4_bounded5_relight_20260724T151711Z/s4_gate.json`：GT reach **1/5**、grasp **0/5**、lift **0/5**、`gate_pass=false` → **Hold**（不扩种子、不重训）。首轮近黑场景运行 `evidence/smolvla_s4_bounded5_20260724T203700Z/`（reach 3/5、grasp 1/5）已标注 **Superseded / historical**，其 reach/grasp 为失明走廊几何重叠 + `GRIPPER_CLOSE_MAX=0.70` 口径放大，不得作权威、不得写成部分成功。归因倾向闭环 BC（H2）；物理链（oracle 5/5）、接口（interface 5/5、150/150 未限幅）、`state[15]` 编码（H3）、相机失明（H1′，修光后仍 Hold）均已排除，但训练域 MuJoCo 对照为人工提前停止的 1-seed，**不得写成协变量偏移已被完全证明**。Online runner：上游 `scripts/run_isaac_smolvla_s4.sh`。证据：`configs/smolvla_s3/eval_gate_v3.yaml`、`configs/smolvla_s3/eval_gate_v3.lock.json`、`runs/smolvla_s3/openloop_recovery_v3_prospective_eval10_gate_v3_20260724T065300Z/s3_open_loop_report.json`、`evidence/smolvla_s4_bounded5_relight_20260724T151711Z/s4_gate.json`、`docs/SMOLVLA_S4_LIFT0_OFFLINE_ATTRIBUTION.md`、`docs/portfolio/SMOLVLA_RECOVERY_V3_PORTFOLIO.md`。
- 2026-07-25 P0 收口（不改任何历史 JSON 数字）：事实一致性审计与作品集材料已完成。统一入口 `docs/portfolio/FINAL_PROJECT_SUMMARY.md`；分层归因 `docs/portfolio/BADCASE_ATTRIBUTION_SUMMARY.md`；后续路线 `docs/FUTURE_WORK_ROADMAP.md`（**P1 / P2 仅登记，不得自动执行**）；求职材料 `docs/portfolio/resume_description.md`。项目统一定位：**面向机器人模仿学习与 VLA 策略的数据质量、动作契约、open-loop、closed-loop、仿真执行和 Badcase 归因评测框架**。旧 Gate / Hold / 失效实验一律保留并标注 `Historical / Invalidated / Superseded / Archived`，不得删除。
...
* **当前接力点（2026-07-25，P0 已收口）**：SmolVLA Recovery v3 **open-loop Pass**（`eval_gate_v3`）；有界 Isaac S4 seeds 1–5 **已跑**（`ran_isaac=true`；lift 0/5 → Hold；权威证据 `evidence/smolvla_s4_bounded5_relight_20260724T151711Z/`，首轮 `evidence/smolvla_s4_bounded5_20260724T203700Z/` 为 Superseded；见 `docs/SMOLVLA_S3_ISAAC_S4_RUN_CHECKLIST.md`）。作品集收口入口：`docs/portfolio/FINAL_PROJECT_SUMMARY.md`；后续 P1/P2 见 `docs/FUTURE_WORK_ROADMAP.md`，**仅登记不执行**。
