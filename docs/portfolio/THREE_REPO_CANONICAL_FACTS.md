...
| SmolVLA Recovery v3 | train-only 36 episodes、`state[15]+scene RGB`、chunk10/K5、官方精确 PEFT 正则完成 5,705-step LoRA，checkpoint audit Pass。历史 14 条 / 3,413 帧 canonical first-action 在冻结 `eval_gate_v1` 下为 Hold。2026-07-24 新采 10 条 / 2,673 帧 eval-only scene episode，QA/release Pass；真实 RTX 4090 D prospective `eval_gate_v2` 为 **Hold**（仅开爪边 raw 严重度三项）。同日冻结执行语义 `eval_gate_v3` 后，fresh seeds 70–74 / 2,593 帧 prospective **Pass**（EE `0.0253 m`、grip BA `0.9943`、close-edge beyond-ε `0.386%`）。有界 Isaac S4（≤5 seeds）已在本机 RTX PRO 500 跑完：`ran_isaac=true`；policy interface 5/5 PASS；GT reach 3/5、grasp 1/5、lift **0/5** → **Hold**。CPU queue 合同 + online runner 已落地 | `implemented_and_verified` for train/checkpoint/v1–v3 gates/prospective v3 Pass + bounded S4 Isaac run；**不是**任务成功 / Sim2Real | `runs/smolvla_s3/openloop_recovery_v3_prospective_eval10_gate_v3_20260724T065300Z/s3_open_loop_report.json`, `evidence/smolvla_s4_bounded5_20260724T203700Z/s4_gate.json`, `configs/smolvla_s3/eval_gate_v3.yaml`, `training/smolvla_s3/runtime_s4.py`, `docs/portfolio/SMOLVLA_RECOVERY_V3_PORTFOLIO.md`, `docs/SMOLVLA_S3_ISAAC_S4_RUN_CHECKLIST.md` |
| ACT HOME_NO_CLOSE hold | 假设—证据矩阵；禁止盲训/扩采/E4 | `implemented_and_verified` decision | `docs/ACT_HOME_NO_CLOSE_HYPOTHESIS_MATRIX.md` |

### VLA 候选路线状态（2026-07-24）
...
lift **0/5** → **Hold**（证据 `evidence/smolvla_s4_bounded5_20260724T203700Z/`）。LingBot **执行路线已归档**；
SmolVLA 为**唯一活动预训练候选**。不是 ACT 重训，也不是 VLA 抓取已验证 / Sim2Real。
不得把“ACT run 已完成”写成“ACT 抓取成功”；不得把“S2 接口 Pass”、 “S3 Ready”、open-loop Pass、
S4 interface Pass 或 `ran_isaac=true` 写成“SmolVLA 已适配 Panda / 任务成功 / Sim2Real”。
