...
与 `design_10day.md` / `hal_ik_roadmap.md` 的关系：

- `baseline_plan.md` 描述当前已经跑通或正在巩固的 V0/V1/V2 数据闭环基线。
- `design_10day.md` 描述基线完成后的 10 天游标架构，是后续增强方向。
- `hal_ik_roadmap.md` 拆解 `design_10day.md` 的 Phase 1，即在当前数据闭环之上新增 HAL、
  IK 和笛卡尔插补能力。
- 当前数据格式以 `../dev/data_schema.md` 和已验证脚本为准，后续增强必须保持
  兼容。
...
RRT、夹爪闭环、批量采集和 LeRobot 标准化导出属于 `design_10day.md` 描述的后续
增强阶段。
...
阶段达到验收标准。之后可以进入 `hal_ik_roadmap.md` 中的 HAL + IK + Cartesian
Interpolation 增强阶段。
...
| Phase 1 HAL/IK/笛卡尔 | W1 后 2–3 天 | 见 `hal_ik_roadmap.md` | 是 |
...
- `../portfolio/interview_walkthrough.md`：3–5 分钟面试讲稿
- README：架构图、Skills 矩阵、Quick Start ≤ 3 命令
- `../reference/migration_ros2_moveit.md`：`RobotControl` HAL → ros2_control / MoveIt
