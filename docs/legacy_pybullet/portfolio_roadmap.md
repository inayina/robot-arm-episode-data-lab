...
本文档是 `baseline_plan.md` 的增补计划，面向 **控制 / 具身 AI / 仿真数据 / 机器人软件**
等多方向同时投递。原则：**先保证可复现与可展示，再叠控制深度与数据规模**。

与现有文档的关系：

- `baseline_plan.md` V0/V1/V2：已完成的作品集基线，不再推翻。
- `hal_ik_roadmap.md` Phase 1：HAL + IK + 笛卡尔插补，广撒网 **必做**。
...
与 `hal_ik_roadmap.md` 完全一致，广撒网 **全部必做**。完成后默认仍保留 `joint_position` 模式。
...
| ID | 任务 | 产出 |
|----|------|------|
| 3.1 | 面试讲稿 | `../portfolio/interview_walkthrough.md` |
| 3.2 | 架构图 | README 内 mermaid 或 `assets/diagrams/architecture.png` |
| 3.3 | 一键复现块 | README「Quick Start」≤ 3 条命令 |
| 3.4 | 迁移设计 | `../reference/migration_ros2_moveit.md` | HAL → ros2_control / MoveIt 映射，**文档即可** |
...
- `../portfolio/interview_walkthrough.md` 覆盖：问题、设计、命令、局限、扩展
...
- [x] V0/V1/V2 基线（已完成）
- [x] Phase 0.5：CI + pytest + config + README GIF
- [x] Phase 1：HAL + IK + cartesian_ik episode
- [x] Phase 1.5：至少 1 条 `success: true` 的 pick/lift GIF
- [x] Phase 2：≥ 20 episode + LeRobot 导出骨架可加载
- [x] Phase 3：interview_walkthrough + migration 设计文档
...
| 岗位倾向 | 在最低线之上优先加 |
|----------|-------------------|
| 运动规划 / 控制 | [rrt_roadmap.md](rrt_roadmap.md) RRT demo；`run_cartesian_demo` 误差日志 |
...
自动进度见 `README.md` / `baseline_plan.md` / `../portfolio/project_status.md` 中的快照区块。
手动勾选本文档后，运行：
