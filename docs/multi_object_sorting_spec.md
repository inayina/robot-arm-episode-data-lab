...
### 5.1 数据校验验证
- 运行 `python3 training/scripts/inspect_dataset.py --schema configs/robot_schemas/panda_multi_task.yaml`，必须验证 `manifest.has_language_instruction=true` 时每帧 `language_instruction` 字段存在且内容不为空。

### 5.2 策略训练与烟雾测试
- CI / 无 GPU 环境运行 `python3 training/scripts/train_act_smoke.py`，验证 dataset → checkpoint → metrics 的轻量线性 baseline 闭环。
- LeRobot 训练环境运行 `python3 training/scripts/train_act_lerobot.py --schema configs/robot_schemas/panda_multi_task.yaml --text-encoder clip`，验证语言嵌入、ACT action chunk 与 checkpoint 输出链路。
