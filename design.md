# 轻量化机械臂闭环仿真平台 - 系统设计文档 (design.md)

## 1. 项目定位与核心原则

本文档描述的是作品集基线完成后的增强版系统设计。当前可运行基线以
`PLAN.md`、`docs/data_schema.md` 和 `scripts/collect_episode.py` 为准；
...
* `metadata.json`：存储全局元数据。当前必要字段以 `docs/data_schema.md` 为准；语言指令（Language Instruction）、任务成功/失败标签（Success Label）和失败原因分类（Error Type）属于后续自动评测与批量采集阶段的扩展字段。
...
[docs/portfolio_roadmap_broad.md](docs/portfolio_roadmap_broad.md)。该路径将 Phase 0.5
