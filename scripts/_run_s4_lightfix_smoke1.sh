#!/usr/bin/env bash
# Single-seed smoke after offline lighting fix (H1').
# Confirms policy-input JPEG is no longer near-black. Does not expand seeds.
set -uo pipefail

MIDSTREAM="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
UPSTREAM="${UPSTREAM:-/home/ina/dev/ros2-arm-teleoperation-suite}"
OUT="${1:-${MIDSTREAM}/evidence/smolvla_s4_lightfix_smoke1_$(date -u +%Y%m%dT%H%M%SZ)}"
LOG="${MIDSTREAM}/evidence/_lightfix_smoke_$(basename "${OUT}").log"

mkdir -p "${OUT}"
echo "${OUT}" > /tmp/smolvla_s4_lightfix_suite_out.txt
echo "${LOG}" > /tmp/smolvla_s4_lightfix_log.txt
...
{
  echo "[lightfix-smoke] start $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  echo "[lightfix-smoke] OUT=${OUT} SEEDS=${SEEDS}"
  bash "${MIDSTREAM}/scripts/run_smolvla_s4_bounded_isaac.sh" "${OUT}"
  status=$?
  echo "[lightfix-smoke] suite_exit=${status} $(date -u +%Y-%m-%dT%H:%M:%SZ)"
  bash "${UPSTREAM}/scripts/stop_stack.sh" >/dev/null 2>&1 || true
  pkill -9 -f '[i]saac_panda_backend' 2>/dev/null || true
  pkill -9 -f '[s]molvla_policy_inference' 2>/dev/null || true
  pkill -9 -f '[i]saac_scene_video' 2>/dev/null || true
